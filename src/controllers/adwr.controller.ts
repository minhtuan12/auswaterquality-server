...
class ADWRController {
  ...
  async parameterBarChart(
    ...
    try {
      ...
      const paramPath = `${isHealth ? 'healthParameters' : 'aestheticParameters'}.${parameterName}`
      ...
      const result = records.map(record => {
        const param = isHealth ? record.healthParameters?.[parameterName] : record.aestheticParameters?.[parameterName]

        if (parameterName === 'Ecoli') {
          return {
            year: record.year,
            pct: param?.pct ?? 0,
            spls: param?.spls ?? 0,
            spls_excd: param?.spls_excd ?? 0,
            spls_pass: param?.spls_pass ?? 0,
          }
        }
